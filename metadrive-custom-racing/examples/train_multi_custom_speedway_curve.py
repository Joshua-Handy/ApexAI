# -*- coding: utf-8 -*-
"""
Train multiple agents on the custom speedway track with SIMPLE CLEAN REWARD SYSTEM.

Current reward system (F13 - simplified, no velocity delta):
- POSITIVE: +20.0 * progress (main driver), +1.0 * completion, +1.0 * speed_normalized
- PENALTIES: -20.0 off-road, -20.0 crash, -20.0 white-line, -0.02 time penalty
- STRICT BOUNDARIES: out_of_road_done=True (terminates on track exit)
- FOCUS: Learn racing naturally through progress optimization

Usage:
    python train_multi_custom_speedway_curve.py --agent-names F13 --timesteps 500000
    python train_multi_custom_speedway_curve.py --num-agents 2 --agent-names F13 F14 --timesteps 500000
"""
# Fix Windows console encoding FIRST - before any imports that might capture stdout
import sys
...
# Disable wandb console capture to avoid encoding conflicts
os.environ['WANDB_CONSOLE'] = 'off'
os.environ['WANDB_SILENT'] = 'true'  # Suppress wandb output entirely
...
def make_env(track_name: str = 'custom_speedway_curve', seed: int = None, agent_id: int = 0, num_agents: int = 2,
             ghost_mode: bool = False, other_agent_models=None) -> Callable[[], object]:
    def _init():
        # Create multi-agent environment with CURVE-AWARE rewards
        ...
        env_config = {
            ...
            # STRICT: Terminate on track exit to prevent exploits
            'crash_vehicle_done': True,
            'crash_object_done': True,
            'out_of_road_done': True,  # CRITICAL for curve-aware training

            # FIXED SPAWN LANES: MetaDrive automatically spawns agents in separate lanes
            # agent0 in lane 0 (left), agent1 in lane 1 (right)
            ...
            # IMPORTANT: Enable terrain and sidewalk for visibility
            # Must match test environment settings!
            'show_terrain': True,
            'show_sidewalk': True,
        }
        base = MultiAgentCustomSpeedwayCurveEnv(env_config)
...
def train_single_agent(
    ...
    try:
        ...
        # Clean up old tensorboard directories to prevent corruption issues
        if os.path.exists(agent_tb_log):
            import shutil
            try:
                shutil.rmtree(agent_tb_log)
                print(f"🧹 Cleaned old tensorboard directory for {agent_name}")
            except Exception as clean_err:
                print(f"⚠️  Could not clean tensorboard directory: {clean_err}")
        ...
        if pretrained_path and os.path.exists(pretrained_path):
            print(f"[FINE-TUNING] Loading pretrained model from: {pretrained_path}")
            # Load without env first
            model = PPO.load(pretrained_path)

            # Fix observation space mismatch - update model's obs space to match environment
            # The shapes are the same (91,), only bounds differ - neural network doesn't care
            print(f"[FINE-TUNING] Updating observation space: {model.observation_space} -> {vec_env.observation_space}")
            model.observation_space = vec_env.observation_space
            model.policy.observation_space = vec_env.observation_space

            # Now set environment (spaces should match)
            model.set_env(vec_env)
            model.tensorboard_log = agent_tb_log
            print(f"[FINE-TUNING] Pretrained model loaded successfully!")
            print(f"[FINE-TUNING] Continuing training for {timesteps:,} more timesteps")
        else:
            if pretrained_path:
                print(f"[WARNING] Pretrained path specified but not found: {pretrained_path}")
                print(f"[WARNING] Training from scratch instead")
            ...
            model = PPO(
                ...
                clip_range=clip_range,
                seed=seed + agent_id,
                ent_coef=0.1,  # Moderate exploration (with improved rewards, less randomness needed)
        ...
        try:
            ...
            # CRITICAL: Close MetaDrive engine to allow next agent to train
            # Without this, the next agent will fail with "engine already initialized"
            from metadrive.engine.engine_utils import close_engine
            close_engine()
            print("🔧 MetaDrive engine closed successfully")
        except Exception as e:
            print(f"⚠️  Error closing environment: {e}")
        
        # CRITICAL: Close MetaDrive engine to allow next agent to initialize
        try:
            from metadrive.engine.engine_utils import close_engine
            close_engine()
            print(f"🔧 MetaDrive engine closed for {agent_name}")
        except Exception as e:
            print(f"⚠️  Error closing MetaDrive engine: {e}")
        ...
        # CRITICAL: Clean up even on failure so next agent can train
        try:
            if 'vec_env' in locals():
                vec_env.close()
            if 'eval_callback' in locals() and eval_callback and hasattr(eval_callback, 'eval_env'):
                eval_callback.eval_env.close()
        except Exception as cleanup_err:
            print(f"⚠️  Error during cleanup: {cleanup_err}")
        
        # Close MetaDrive engine
        try:
            from metadrive.engine.engine_utils import close_engine
            close_engine()
            print(f"🔧 MetaDrive engine closed after failure for {agent_name}")
        except Exception as engine_err:
            print(f"⚠️  Error closing engine: {engine_err}")
        ...
        try:
            ...
            print(f"⚠️  Error finishing wandb: {wandb_err}")
        
        if result_queue:
            result_queue.put({'agent_name': agent_name, 'success': False, 'error': str(e)})
        
        # Don't re-raise - let next agent try to train
        return None
...
def main():
    ...
    parser.add_argument('--pretrained', type=str, default=None,
                       help='Path to pretrained model to fine-tune (e.g., results/results_agent_F16/F16_custom_speedway.zip)')
    ...
    # Auto-confirm for non-interactive environments (like Colab)
    parser.add_argument('--yes', '-y', action='store_true', help='Skip confirmation prompts (for Colab/scripts)')
    ...
    print("\n" + "="*70)
    print("MULTI-AGENT CUSTOM SPEEDWAY TRAINING (AGENT_0V2 REWARDS)")
    print("="*70)
    ...
    print("\nReward System: F13 Simple Clean Formula")
    print("  +20.0*progress | +1.0*completion | +1.0*speed | -0.02 time")
    print("  -20 off-road | -20 crash | -20 white-line")
    print("  STRICT boundaries (out_of_road_done=True)")
    print("="*70 + "\n")
    
    # Confirm if training many timesteps (skip if --yes flag is used for Colab/scripts)
    if args.timesteps > 100000 and not args.yes:
        ...
        for i, agent_name in enumerate(agent_names):
            try:
                save_path = train_single_agent(
                    ...
                    args.eval_freq, args.checkpoint_freq, args.no_eval,
                    args.wandb, args.wandb_project, args.wandb_entity,
                    args.ghost_mode, args.other_agent_model, args.pretrained, None
                )
