"""
Single-agent environment that builds a straight track at runtime using PGBlocks.

This creates a simple straight track with the same characteristics as the oval track.
"""
...
class SingleAgentStraightMap(PGMap):
    """A minimal PGMap that creates a single straight track.

    The parameters (lane_num, lane_width) match the oval track configuration.
    """

    def _generate(self):
        parent_node_path, physics_world = self.engine.worldNP, self.engine.physics_world
        assert len(self.road_network.graph) == 0, "Map is not empty; create a fresh map to build the straight track"
        ...
        # Add a single long straight block
        ...
        straight_block.construct_from_config(
            {
                Parameter.length: 1000,  # Long straight section
            },
            parent_node_path,
            physics_world
        )
...
class SingleAgentStraightMapManager(PGMapManager):
    """A map manager that always loads SingleAgentStraightMap once."""

    def reset(self):
        config = self.engine.global_config
        if len(self.spawned_objects) == 0:
            _map = self.spawn_object(SingleAgentStraightMap, map_config=config["map_config"], random_seed=None)
        else:
...
class SingleAgentStraightEnv(MetaDriveEnv):
    """Drop-in MetaDriveEnv that uses SingleAgentStraightMapManager to build a straight track."""

    def setup_engine(self):
        super().setup_engine()
        # Replace the default PGMapManager with our straight track builder
        self.engine.update_manager("map_manager", SingleAgentStraightMapManager())
    ...
    def done_function(self, vehicle_id: str):
        ...
        # Check if vehicle is in void (lost contact with any road/lane surface)
        in_void = "GROUND" in vehicle.contact_results
        
        # Episode ends if there's a solid crash OR lane line collision OR vehicle is in void
        done = solid_crash or lane_line_collision or in_void
