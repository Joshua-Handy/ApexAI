...
def main():
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        # CRITICAL FIX: Update hyperparameters for resumed model
        print(f'Updating hyperparameters for resumed model:')
        print(f'  Learning rate: {args.learning_rate}')
        print(f'  Batch size: {args.batch_size}')
        print(f'  Clip range: {args.clip_range}')
        
        # Update the learning rate (CRITICAL: Use SB3's built-in method)
        print(f"🔧 Setting learning rate via SB3 method: {args.learning_rate}")
        model.lr_schedule = lambda _: args.learning_rate  # Set the learning rate schedule
        model.learning_rate = args.learning_rate  # Set the attribute
        
        # CRITICAL: Use SB3's _setup_learn method to properly update optimizers
        # This ensures ALL optimizers (policy and value) get the new learning rate
        model._setup_model()
        print(f"✅ All optimizers updated with learning rate: {args.learning_rate}")
        
        # Update clip range (need to handle callable vs float)
        if callable(args.clip_range):
            model.clip_range = args.clip_range
        else:
            # Convert float to constant function
        ...
        # CRITICAL FIX: Reset the rollout buffer to match new environment and hyperparameters
        # This prevents the AssertionError when the buffer size doesn't match
        ...
        print("✅ Rollout buffer reset for resumed training")
