...
def main():
    ...
    if wandb_run and WANDB_AVAILABLE:
        # Adjust gradient_save_freq to account for parallel environments
        grad_save_freq = max(1000 // args.num_envs, 1)
        # Don't save model through WandB to avoid permission issues
        wandb_callback = WandbCallback(
            gradient_save_freq=grad_save_freq,
            verbose=2,
            model_save_path=None  # Disable model saving through wandb
        )
        ...
        callbacks.extend([wandb_callback, racing_callback])    # When resuming, continue counting timesteps
