...
class SingleAgentWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # Get actual agent IDs from the current observation space
        actual_agent_ids = list(self.env.observation_space.spaces.keys())
...
def make_env(track_name: str = 'custom_speedway', seed: int = None, agent_id: int = 0, num_agents: int = 2) -> Callable[[], object]:
    def _init():
        ...
        env_config = {
            ...
            # REAL RACETRACK: Crashes end episodes
            'crash_vehicle_done': True,   # Crash into car = done
            'crash_object_done': True,    # Hit barriers = done
            'out_of_road_done': False,    # Allow some off-road with penalty
