...
class SingleAgentWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        actions = {}
        for aid in self.env.observation_space.spaces.keys():
...
def make_env(track_name: str = 'custom_speedway', seed: int = None, agent_id: int = 0, num_agents: int = 2) -> Callable[[], object]:
    def _init():
        ...
        env_config = {
            ...
            'crash_vehicle_done': False,  # Don't terminate on crash - let agent learn
            'crash_object_done': False,
            'out_of_road_done': False,  # Don't terminate for off-road
            'horizon': 2000,  # Longer episodes
...
def main():
    ...
    # Setup agent names
    if args.agent_names:
        if len(args.agent_names) != args.num_agents:
            print(f"⚠️  Warning: {len(args.agent_names)} names provided but {args.num_agents} agents requested.")
            print(f"    Using first {args.num_agents} names or filling with defaults.")
        agent_names = args.agent_names[:args.num_agents]
        # Fill remaining with defaults if needed
        for i in range(len(agent_names), args.num_agents):
            agent_names.append(f'agent_{i}')
    else:
